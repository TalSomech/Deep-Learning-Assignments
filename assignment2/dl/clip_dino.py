...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    best_class_indices = similarities.argmax(dim=0)

    pred_classes = [class_texts[idx] for idx in best_class_indices.cpu()]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.images = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
        self.image_features = self.clip_model.encode_image(self.images)
        self.device = device
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        query_sim = get_similarity_no_loop(text_features, self.image_features)

        query_sim = query_sim.squeeze(0)

        top_indices = torch.topk(query_sim, k).indices.cpu().tolist()
